...
# Declare all items
gold_coins = Item("Coins", "Money")
sword = Item("Sword", "It's a sword")
bag = Item("bag", "This is a bag")
hat = Item("Hat", "a hat to wear")
rocks = Item("Rocks", "Rock you can throw")
necklace = Item("necklace", "A necklace to look fancy")


# Add Items to rooms
room['outside'].items = [gold_coins, rocks]
room['foyer'].items = [sword]
room['overlook'].items = [bag]
room['narrow'].items = [hat]
room['treasure'].items = [necklace]
...
def check_command(command):
    ...
    if len(splt_command) == 1:
        if command == "n" or command == "e" or command == "s" or command == "w":
            ...
            clear_console()
            print("Please make sure you are enter a valid command.")
            input('Press any key to continue...')
